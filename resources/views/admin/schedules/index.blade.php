                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="form-group col-md-12">
                            ...
                            <input type="text" name="due" id="due_edit" class="form-control form-control-sm date" />
                            <span class="invalid-feedback"></span>
                        </div>
                        <div class="form-group col-md-12">
                            <label for="file_id" class="required">File</label>
                            <div class="input-group">
                                <select name="file_id" id="file_id_edit" class="form-control form-control-sm select2"  style="width: 100%;">
                                    <option value="">--- Select File ---</option>
                                    @foreach ($files as $file)
                                        <option value="{{$file->id}}">{{ $file->fileId}}-{{ $file->title_of_content}}</option>
                                    @endforeach
...
<script>
    function editModal(id)
    {
        ...
        $.get('../../../admin/schedules/'+id+'/edit', function(response){
            $('#due_edit').val(response.schedule_due);
            $('#file_id_edit').val(response.file_id).change();
        ...
        $('#editForm').submit(function(e){
            ...
            let due = $('#due_edit').val();
            let file_id = $('#file_id_edit').val();
            let position = $('#position_edit').val();
            let _token = $('input[name="_token"]').val();
